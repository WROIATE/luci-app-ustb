...
local nat = luci.sys.exec("uci get ustb.@advance[0].nat")
if ( nat == "0" )
then
	button_add_nat6 = s:option (Button, "_button_add_nat6", translate("添加IPV6 Nat6防火墙"),translate("正常只需点击一次，请确保WAN6已获得V6地址"))
	button_add_nat6.inputstyle = "apply" 
	button_add_nat6.inputtitle = translate ("点击添加")
else 
	button_del_nat6 = s:option (Button, "_button_del_nat6", translate("删除IPV6 Nat6防火墙"),translate("正常只需点击一次，删除USTB规则"))
